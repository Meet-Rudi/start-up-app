"""
MEET_RUDI — structured call record for meetrudi-call (real telephony).

Every bench call leaves an analysable trail in the shared data bucket:

    calls/{call_id}/manifest.json          config, state, running totals, outcome
    calls/{call_id}/turns/{seq:04d}.json    one object per turn — never appended to, so turns
                                            can't race or truncate each other
    calls/{call_id}/feedback/{seq:02d}.json operator notes
    calls/_index/{YYYY-MM-DD}/{call_id}.json  one light row per call, for listing

Audio is NOT written here. Twilio holds the media on a real call and we never receive it, which
is the right default for patient data — the transcript is what we need and the recording is what
...
# Real patient calls live under their own prefix, separate from the bench, so a
# retention or erasure sweep can treat them differently without filtering.
PREFIX = os.environ.get("CALL_PREFIX", "calls")
...
def _manifest_key(call_id):
    return "%s/%s/manifest.json" % (PREFIX, call_id)


def _turn_key(call_id, seq):
    return "%s/%s/turns/%04d.json" % (PREFIX, call_id, seq)


def audio_key(call_id, seq, who, ext):
    return "%s/%s/audio/%04d-%s.%s" % (PREFIX, call_id, seq, who, ext)


def _index_key(call_id, started_at):
    day = (started_at or iso())[:10]
    return "%s/_index/%s/%s.json" % (PREFIX, day, call_id)
